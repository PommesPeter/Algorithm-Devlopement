...
- 情况1
    1. `mid = l + r >> 1`
    2. if (check(mid)) (区间被划分成\[l, mid\]和\[mid + 1, r\]) (左边为满足性质的，右边不满足性质)
        true: \[l, mid\]; r = mid; 如果check返回true，说明mid满足了性质，那么mid一定会落在红色区域，所以答案肯定包含mid，也就是在\[l, mid\], 更新左边界为r = mid; 缩小区间。
        false: \[mid + 1, r\]; l = mid + 1; 如果check返回false，说明mid不满足性质，那么mid会落在绿色的区域，此时区间为\[mid + 1, r\], 更新右边界为l = mid + 1;
    
- 情况2
    1. `mid = l + r + 1 >> 1`
    2. if (check(mid)) (区间被划分成\[l, mid - 1\]和\[mid, r\]) (右边为满足性质的，左边不满足性质)
       true: \[l, mid - 1\]; l = mid; 如果check返回true，说明mid满足了性质，那么mid一定会落在绿色区域，所以答案肯定包含mid，也就是在\[mid, r\], 更新左边界为l = mid; 缩小区间。
       false: \[mid, r\]; r = mid - 1; 如果check返回false，说明mid不满足性质，那么mid会落在红色区域，此时区间为\[l, mid - 1\], 更新右边界为r = mid - 1;

会产生这两种情况是因为mid落在正确区间的解不一样，二分的方向不一样。       

> 思考顺序: 确定边界，写好模板，然后根据问题的性质，根据图形（画个数轴）得到二分的边界，性质是什么，再写判断，再用图对比。（check是true的话，答案应该在哪个边界，如果是false又在哪个边界。）
...
平时常用归并排序、快速排序都是双指针算法。双指针算法主要有两种应用：双指针维护两个数组；双指针维护一个数组。常规的双重循环暴力遍历复杂度为$O(n^2)$，双指针算法的目的就是为了优化这种暴力遍历的算法，一般优化到$O(n)$
...
离散化主要解决的问题特点是输入规模可能比较大，值域比较大(0-10^9)，但是个数比较少(10^5)。把这么大规模的数映射到一个从0开始的自然数。也就是将一个数组里的数映射到一个从0开始的自然数的数组。可能有重复的元素，所以需要去重。
...
- unique函数实现原理——双指针算法: 排完序之后：1. 他是第一个；2. 他和前面一个数不一样，`a[i] != a[i - 1]`。只要满足这两个性质就把这个数拿出来就行
...
```
- 如何计算一个数组a离散化之后的结果是多少？
  可以使用二分，找到需要进行离散化的元素，然后进行映射。
## 区间合并

有多个区间，如果彼此之间有交集的话就进行合并，合并成同一个区间，并集作为一个区间。

> 整体思想: 对左端点把区间进行排序，然后开始扫描，扫描的时候有两个指针st,ed，扫描的时候从第一个区间开始，判断第一个区间和第二个区间有没有公共点，如果有，就把第一个区间和第二个区间进行合并(合并的时候只需要把下一个区间的右端点赋值给ed即可，ed表示第i个区间的右端点)，判断完之后再看下一个区间（**从这里可以看出需要有一个pair存区间的左右端点**）。如果不产生任何交集就到此为止了。然后依次往后做。
>
...
2. 扫整个数组，有两个指针st,ed;
   ![image-20210606220050178](./src/BasicAlgorithm/image-20210606220050178.png)